package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1197:Number = 1.0;
      
      private var var_638:BitmapData;
      
      private var var_784:ColorTransform;
      
      private var var_1465:ColorTransform;
      
      private var var_1906:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_679:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_784 = new ColorTransform();
         this.var_784.alphaMultiplier = 1;
         this.var_1465 = new ColorTransform();
         this.var_1906 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1196)
         {
            _loc1_.dispose();
         }
         this.var_1196 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_95 = this.var_1196[param1];
         this.var_971 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1905 >= 0)
         {
            this.var_266 = this._visualization.getSprite(this.var_1905);
            if(this.var_266 && this.var_266.asset)
            {
               ...
               if(this.var_85 == null)
               {
                  this.var_85 = this.var_266.asset.clone();
                  if(this.var_784.alphaMultiplier != 1)
                  {
                     this.var_638 = new BitmapData(this.var_85.width,this.var_85.height,true,4278190080);
                  }
               }
               this.var_2567 = -this.var_266.offsetX;
               this.var_2568 = -this.var_266.offsetY;
               this.var_266.asset = this.var_85;
            }
            if(this.var_85)
            {
            ...
            if(this.var_638)
            {
               this.var_638.fillRect(this.var_638.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_95.y * this.var_1197;
         }
         return 0;
      }
      ...
      {
         if(!this.var_95 || !this.var_266)
         {
            return;
         }
         if(this.var_85 && this.var_266.asset != this.var_85)
         {
            this.var_266.asset = this.var_85;
         }
         if(this.var_971)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_971 && this.var_95.hasIgnited)
         {
            this.var_971 = true;
         }
         _loc8_ = this.var_1075 * this.var_1197;
         this.var_95.update();
         if(this.var_971)
         {
            if(this.var_95.roomObjectSpriteId >= 0)
            ...
            {
               this.var_85.draw(this.var_638,this.var_1906,this.var_784,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_95.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2567 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1197;
               _loc4_ = this.var_2568 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1197;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1465.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_85.draw(_loc7_,this._translationMatrix,this.var_1465,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2700 = parseInt(param1.@size);
         this.var_1905 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1075 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1197 = this.var_2700 / 64;
         this.var_679 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_679 = Math.min(this.var_679,1);
         this.var_784.alphaMultiplier = this.var_679;

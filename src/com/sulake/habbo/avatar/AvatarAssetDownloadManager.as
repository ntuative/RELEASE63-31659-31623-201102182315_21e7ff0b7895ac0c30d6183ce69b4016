package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_833:int = 3;
      
      private var var_387:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_387 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_835 = new Timer(this.const_1950,1);
         this.var_835.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_387)
         {
            this.var_387.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_387.addEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_387)
         {
            this.var_387.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_387.removeEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_833);
            if(this.var_1283.indexOf("?") > 0)
            {
               _loc2_ = this.var_1283 + "&retry=" + this.var_833;
            }
            else
            {
               _loc2_ = this.var_1283 + "?retry=" + this.var_833;
            ...
            this.var_387 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2166,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1040[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1040[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1568)
         {
            this.var_1039.push([param1,param2]);
            return;
         }
      ...
      {
         var _loc1_:* = null;
         while(this.var_677.length > 0 && this.var_676.length < this.const_1426)
         {
            _loc1_ = this.var_677[0];
            _loc1_.startDownloading();
            this.var_676.push(this.var_677.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_677.indexOf(param1) == -1 && this.var_676.indexOf(param1) == -1)
         {
